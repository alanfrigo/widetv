...
class ApiClient(private val store: Store) {
  ...
  /**
   * Modo de apresentacao. null quando o servidor e antigo e nao conhece a rota
   * (404) — quem chama trata isso como "continua sendo o CRT de sempre".
   */
  suspend fun config(): ConfigResponse? = withContext(Dispatchers.IO) {
    getBodyOrNull(url(Routes.CONFIG))?.let { json.decodeFromString<ConfigResponse>(it) }
  }
